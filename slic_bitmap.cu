...
__global__ void printBitmapAssoc(int *bitmapAssoc, int nCol, int nRow)
{
  int ix = threadIdx.x + blockIdx.x * blockDim.x;
  int iy = threadIdx.y + blockIdx.y * blockDim.y;
  int idx = iy*nCol + ix;
  ...
  if (bitmapAssoc[idx] >= 0)
    printf("Ix:%d Iy:%d  cluster_id:%d\n", ix, iy, bitmapAssoc[idx]);
}
...
__global__ void initBitmapAssoc(int *bitmapAssoc, int nCol, int nRow)
{
  int ix = threadIdx.x + blockIdx.x * blockDim.x;
  int iy = threadIdx.y + blockIdx.y * blockDim.y;
  int idx = iy*nCol + ix;
...
/*  initClusters: Inizializza ogni cluster, equidistanti SxS.
                  Sposta ogni cluster
*/
__global__ void initClusters(float *pxl_label, float *cluster, int width, int height, int sWidth, int * bitmapAssoc)
{
  ...
  int ix = (((centroid_Id ) % sWidth) * S) + S; // -1
  int iy = (((centroid_Id ) / sWidth) * S) + S; // -1
  ...
  // fai associazione indice su array cluster e griglia immagine
  bitmapAssoc[min_iy * width + min_ix] = centroid_Id;
}
...
*/
__global__ void assignment(float *pxl_label, float *clusters, int nCol, int nRow, int * bitmapAssoc)
{
  int ix = threadIdx.x + blockIdx.x * blockDim.x;
  int iy = threadIdx.y + blockIdx.y * blockDim.y;
  int idx = iy*nCol + ix;
  ...
  int idx_centroid = -1;
  int idx_centroid_now = -1;
  unsigned char p;

  float L_c, A_c, B_c;

  int i = ix-S > 0 ? ix-S : 0;
  int iLimit = i+S < nCol ? i+S : nCol;

  // shared memory per forza... sia su bitmap che su cluster .. forse
  for (; i<iLimit; i++) {

    int j = iy-S > 0 ? iy-S : 0;
    int jLimit = j+S < nRow ? j+S : nRow;

    for (; j<jLimit; j++) {

      idx_centroid_now = bitmapAssoc[j * nCol + i];
      p = idx_centroid_now >= 0;
       
      if ( p ) {
        L_c = clusters[idx_centroid_now];
        A_c = clusters[idx_centroid_now + K];
        B_c = clusters[idx_centroid_now + 2*K];

        L_c = sqrtf(powf(L_c - L, 2) + powf(A_c - A, 2) + powf(B_c - B, 2));  // dc
        A_c = sqrtf(powf(i - ix, 2) + powf(j - iy, 2));                       // ds
        B_c = sqrtf(powf(L_c / M, 2) + powf(A_c / (float)S, 2));              // D
        
        p = B_c > min_dist;
        min_dist = p ? min_dist : B_c;

        idx_centroid = p ? idx_centroid : idx_centroid_now;
      }
    }
  }
...
*/
__global__ void update(float *pxl_label, float *clusters, int nCol, int nRow, int *bitmapAssoc)
{
  ...
  int ccounter_x = 0;
  int ccounter_y = 0;
  int x_c = (int)clusters[idx + 3*K];
  int y_c = (int)clusters[idx + 4*K];


  // inizializza variabili per il loop, così da ridurre divergenze
  int idN;
  char p = 0;
  int i = x_c-S >= 0 ? x_c-S : 0;
  int iLimit = i+S <= nCol ? i+S : nCol;

  // shared memory per pixel SxS più avanti...
  for(; i<iLimit; i++) {

    int j = y_c-S >= 0 ? y_c-S : 0;
    int jLimit = j+S <= nRow ? j+S : nRow;
  ...
  clusters[idx + 3*K] = ccounter_x;
  clusters[idx + 4*K] = ccounter_y;

  // update position cluster on bitmap
  bitmapAssoc[ccounter_y * nCol + ccounter_x] = idx;
  
  //printf("ID:%d old_x:%d old_y:%d ccounter_x:%d ccounter_y:%d ccounter:%d\n", idx, x_c, y_c, ccounter_x, ccounter_y, ccounter);
}
...
int main(int argc, char **argv) {
  ...
  */
  ...
  CHECK(cudaMalloc((int **)&d_bitmapAssoc, nBytes/3 ));
  ...
  initBitmapAssoc<<<grid, block>>>(d_bitmapAssoc, nCol, nRow);
  initClusters<<<(K_h + dimx - 1) / dimx, dimx>>>(d_pxl_lab, d_cluster, nCol, nRow, nCol / S_h, d_bitmapAssoc);
  ...
  for (cc = 1; cc <= nIteration; cc++) {
    ...
    assignment<<<grid, block>>>(d_pxl_lab, d_cluster, nCol, nRow, d_bitmapAssoc);

    if (debug) printf("  update<<< grid(%d,%d) block(%d,%d) >>>\n", (K_h+dimx-1)/dimx, 0,dimx, 0);
    update<<<(K_h + dimx - 1) / dimx, dimx>>>(d_pxl_lab, d_cluster, nCol, nRow, d_bitmapAssoc);
    ...
    if(snapshot && cc < nIteration) {
      ...
      convertLab2RGB<<<grid, block>>>(d_output_tmp, d_pxl_rgb, nCol, nRow);
