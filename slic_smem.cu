...
__global__ void update(float *pxl_label, float *clusters, int nCol, int nRow)
{
  ...
  unsigned int idx = iy*nCol + ix;
  unsigned int smem_ix = threadIdx.x;
  unsigned int smem_iy = threadIdx.x;

  // inseriamo shared memory (arch fermi 48k shared giusti giusti per 3 thread block per SM)
  __shared__ float Ls[BLOCK_SIZE][BLOCK_SIZE];
  __shared__ float As[BLOCK_SIZE][BLOCK_SIZE];
  __shared__ float Bs[BLOCK_SIZE][BLOCK_SIZE];
  __shared__ float Labels[BLOCK_SIZE][BLOCK_SIZE];

  // ogni thread carica il proprio pixel e label
  Ls[smem_iy][smem_ix] = pxl_label[idx];
  As[smem_iy][smem_ix] = pxl_label[idx + N];
  Bs[smem_iy][smem_ix] = pxl_label[idx + 2*N];
  Labels[smem_iy][smem_ix] = pxl_label[idx + 3*N];

  __syncthreads(); 
  
  
  // soltanto il thread 0 per ogni warp fa la sum reduction parallela 
  
  if (! smem_ix ) {

    // Limit K = 2000, e quindi 48K per local memory....
    float LAB_sum [6*K_limit] = {0};
    int centroid_tmp, ix_tmp, iy_tmp;

    int blockSize_iy = blockIdx.y * blockDim.y;
    int blockSize_ix = blockIdx.x * blockDim.x;

    int j_limit = ((BLOCK_SIZE - 1) + blockSize_iy ) - nRow;
    int i_limit = ((BLOCK_SIZE - 1) + blockSize_ix ) - nCol;
    j_limit = j_limit < 0 ? blockDim.y : blockDim.y - (j_limit + 1);
    i_limit = i_limit < 0 ? blockDim.x : blockDim.x - (i_limit + 1);
    
    // calcola rispettive somme parziali
    for(int j=0; j<j_limit; j++) {
      for(int i=0; i<i_limit; i++) {

        centroid_tmp = (int)Labels[j][i];

        if(centroid_tmp > -1) {
          iy_tmp = j + blockSize_iy;
          ix_tmp = i + blockSize_ix;

          LAB_sum[centroid_tmp] += Ls[j][i];
          LAB_sum[centroid_tmp + K_limit] += As[j][i];
          LAB_sum[centroid_tmp + 2*K_limit] += Bs[j][i];
          LAB_sum[centroid_tmp + 3*K_limit] += (float)ix_tmp;
          LAB_sum[centroid_tmp + 4*K_limit] += (float)iy_tmp;
          LAB_sum[centroid_tmp + 5*K_limit] += 1.;
        }

      }
    }
    
    // update atomico dei cluster
    for(int i=0; i < K; i++)
    {
      atomicAdd(&clusters[i], LAB_sum[i]);
      atomicAdd(&clusters[i + K], LAB_sum[i + K_limit]);
      atomicAdd(&clusters[i + 2*K], LAB_sum[i + 2*K_limit]);
      atomicAdd(&clusters[i + 3*K], LAB_sum[i + 3*K_limit]);
      atomicAdd(&clusters[i + 4*K], LAB_sum[i + 4*K_limit]);
      atomicAdd(&clusters[i + 5*K], LAB_sum[i + 5*K_limit]);
    }
  ...
  // A questo punto associamo un'unico update per ogni cluster, usiamo idx che certamente sarà << K
  if (idx < K) {
    unsigned int ccounter = clusters[idx + 5*K];
    clusters[idx] = clusters[idx] / ccounter;
    clusters[idx + K] = clusters[idx + K] / ccounter;
    clusters[idx + 2*K] = clusters[idx + 2*K] / ccounter;
    clusters[idx + 3*K] = (float)( (int) (clusters[idx + 3*K] / ccounter) );
    clusters[idx + 4*K] = (float)( (int) (clusters[idx + 4*K] / ccounter) );
    clusters[idx + 5*K] = 1.;
  }
...
int main(int argc, char **argv) {
  ...
  // check argument 
  unsigned char c = ((K_h <= 0)|(K_h > K_limit)) << 2 | (M_h <= 0.) << 1 | (S_h <= 0);
  if ( c ) {
    if ( c & 1)
  ...
  */
  ...
  CHECK(cudaMalloc((float **)&d_cluster, 6 * K_h * sizeof(float)));
